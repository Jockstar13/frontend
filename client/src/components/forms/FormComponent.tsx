...
const FormComponent = () => {
  ...
  const joinRoom = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (isJoinRequested || status === USER_STATUS.ATTEMPTING_JOIN) return;
  ...
  useEffect(() => {
    const handleUsernameExists = () => {
      setError("Username already exists in room");
      setLoading(false);
 ...
 useEffect(() => {
     ...
     if (status === USER_STATUS.JOINED && !isRedirect) {
       ...
       setIsJoinRequested(false); // Reset the flag
       socket.disconnect();
       socket.connect();
     }
     ...
     <div className="flex w-full max-w-[500px] flex-col items-center justify-center gap-4 p-4 sm:w-[500px] sm:p-8">
       ...
       <form onSubmit={joinRoom} className="flex w-full flex-col gap-4">
         ...
         <button
           ...
           disabled={isJoinRequested} // Disable button while join is in progress
         >
           {isJoinRequested ? 'Joining...' : 'Join'}
         </button>
       </form>
       <button
